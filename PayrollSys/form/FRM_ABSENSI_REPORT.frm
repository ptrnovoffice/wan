...
Begin VB.Form FRM_ABSENSI_REPORT 
   BackColor       =   &H00FFFFFF&
   ...
   End
End
Attribute VB_Name = "FRM_ABSENSI_REPORT"
...
If Index = 1 Then
    ...
    '----- EXPORT REPORT EXCEL ---
    '-----------------------------
    If (CmbRpt(4).Text) = "PRESENSI_DAILY" Then
        PrjExportExcel.Persensi_Daily Format(RptTgl(0).Value, "yyyy-mm-dd"), Format(RptTgl(1).Value, "yyyy-mm-dd"), CmbRpt(0).Text, CmbRpt(1).Text, CmbRpt(2).Text, CmbRpt(3).Text, CmbRpt(4).Text, 1
    
    End If
    If (CmbRpt(4).Text) = "Persensi_Session_Closing" Then
        PrjExportExcel.Persensi_SesionClose
    End If
...
prjSysID.PRESENSI_RPT_LIST CmbRpt(4)
